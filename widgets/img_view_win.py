...
class ZoomBtns(QFrame):
    ...
    def __init__(self, parent: QWidget = None) -> None:
        ...
        def add_btn(name, val):
            btn = UserSvg(os.path.join(Static.in_app_icons_dir, name), 45)
            btn.value = val
...
class PrevImgBtn(SwitchImgBtn):
    def __init__(self, parent: QWidget = None) -> None:
        super().__init__(os.path.join(Static.in_app_icons_dir, "prev.svg"), parent)


class NextImgBtn(SwitchImgBtn):
    def __init__(self, parent: QWidget = None) -> None:
        super().__init__(os.path.join(Static.in_app_icons_dir, "next.svg"), parent)
