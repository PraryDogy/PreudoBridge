...
class CopyFilesTask(URunnable):
    ...
    def task(self):
        ...
        # теперь мы сощдаем nested urls, таким образом будет "имя папка - копия"
        # а внутри папки сохранится нормальная структура файлов
        # и потом мы создаем словарик "исходный путь - путь куда скопировать"
        # и дальше побайтово копируем
